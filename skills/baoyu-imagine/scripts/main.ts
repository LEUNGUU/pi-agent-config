...
const DEFAULT_PROVIDER_RATE_LIMITS: Record<Provider, ProviderRateLimit> = {
  ...
  bedrock: { concurrency: 3, startIntervalMs: 1100 },
};
...
Options:
  ...
  --provider google|openai|openrouter|dashscope|minimax|replicate|jimeng|seedream|azure|bedrock  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
export function getConfiguredProviderRateLimits(
  ...
  const configured: Record<Provider, ProviderRateLimit> = {
    ...
    bedrock: { ...DEFAULT_PROVIDER_RATE_LIMITS.bedrock },
  };

  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "minimax", "jimeng", "seedream", "azure", "bedrock"] as Provider[]) {
...
export function detectProvider(args: CliArgs): Provider {
  ...
  const hasBedrock = (() => { try { require("node:child_process").execFileSync("aws", ["sts", "get-caller-identity", ...(process.env.BEDROCK_PROFILE || process.env.AWS_PROFILE ? ["--profile", process.env.BEDROCK_PROFILE || process.env.AWS_PROFILE] : [])], { encoding: "utf8", timeout: 5000, stdio: "pipe" }); return true; } catch { return false; } })();
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  ...
  if (provider === "bedrock") return (await import("./providers/bedrock")) as ProviderModule;
...
function getModelForProvider(
  ...
  if (extendConfig.default_model) {
    ...
    if (provider === "bedrock" && extendConfig.default_model.bedrock) return extendConfig.default_model.bedrock;
  }
...
async function runBatchTasks(
  ...
  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "jimeng", "seedream", "azure", "bedrock"] as Provider[]) {
    const limit = providerRateLimits[provider];
